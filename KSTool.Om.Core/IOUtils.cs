...
public static class IOUtils
{
    ...
    private static IEnumerable<FileInfo> InnerEnumerate(DirectoryInfo currentDir, string[]? extNames)
    {
        ...
        {
            var subFiles = InnerEnumerate(di, extNames);
    ...
    public static string GetRelativePath(string baseFolder, string filePath)
    {
        ...
        var makeRelativeUri = uri1.MakeRelativeUri(uri2);
        var relativePath = HttpUtility.UrlDecode(makeRelativeUri.OriginalString);
        relativePath = Path.DirectorySeparatorChar == '\\' ? relativePath.Replace('/', '\\') : relativePath;
        return relativePath;
    }
    ...
    public static void DeleteToRecycleBin(this FileInfo file)
    {
        DeleteToRecycleBin(file.FullName);
    }
